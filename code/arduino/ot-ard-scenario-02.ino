...
#define BUZZER_COIL         0x00  // The Modbus coil's offset (Digital output)
#define BUZZER_PIN             4  // The analog input pin
...
void setup() {
  ...
  */
  ...
  /*
     Configure registers - 11 x 32 bits for storing temperature, acceleration and orientation
  */
  mbs.configureInputRegisters(TEMP_SENSOR_IREG, 11);

  /*
     Configure coil - Default is "no allarm is ringing"
  */
  pinMode(BUZZER_PIN, OUTPUT);
  mbs.configureCoils(BUZZER_COIL, 1);
  ...
  */
  if (c) {
    ...
    */
    ...
    /*
        Check if the buzzer coil is high and, if so, start making some noise!
    */
    int buzzer = mbs.coilRead(BUZZER_COIL);

    if (buzzer) {

      Serial.println("[INFO] Alarm started!");
      digitalWrite(4, HIGH);
      delay(50);
