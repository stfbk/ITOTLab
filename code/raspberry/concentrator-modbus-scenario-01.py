...
def read_slave():
	...
	v_dic = {"Temperature": "{}".format(regs_float[0]),
			"Acceleration (X,Y,Z)": "{}".format(regs_float[1:4]),
			"Orientation: (X,Y,Z)": "{}".format(regs[8:])}

	if len(regs):
		print("\tTemperature: {}".format(regs_float[0]))
		print("\tAcceleration: (X,Y,Z): {}".format(regs_float[1:4]))
		print("\tOrientation: (X,Y,Z): {}".format(regs[8:]))
	else:
